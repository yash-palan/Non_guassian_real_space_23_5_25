...
class input_variables:
    ...
    def __init__(self,fermionic_position_array:torch.Tensor,bosonic_position_array:torch.Tensor,fourier_array:torch.Tensor,
        ...
        self.N_b = len(bosonic_position_array)                 # Size : int 
        self.N_f = len(fermionic_position_array)        # Size : int 
        self.J_0 = J_0              # Size : (N_f, N_f) (complex torch tensor)
        self.lmbda = lmbda          # Size : (N_b, N_f) (complex torch tensor)
    ...
    def updating_lambda_from_spin_removed_lambda_bar(self,spin_removed_lambda_bar:torch.Tensor,spin_index=True,update_in_input_variables=True)->torch.Tensor:
        """
        ...
        if(spin_index==True):
            ...
            final_mat = torch.cat((spin_removed_lambda_bar, spin_removed_lambda_bar), dim=1)
        else:
...
class computed_variables:
    ...
    def omega_bar(self,input_variable:input_variables)->torch.Tensor:
        ...
        self.omega_bar_mat = torch.kron(torch.eye(2),omega)
    ...
    def alpha_bar(self,input_variable:input_variables)->torch.Tensor:
        ...
        lambda_temp = torch.transpose(lmbda,0,1)
        temp_mat = lambda_temp.unsqueeze(1) - lambda_temp.unsqueeze(0) 
        final_mat = torch.cat((torch.zeros((N_f,N_f,N_b)),temp_mat) ,dim=2)

        if(final_mat.shape != (N_f,N_f,2*N_b)):
            raise Exception("The shape of the matrix is not correct. Check the code again")
        
        # Converting to a pytorch tensor 
        if(final_mat.dtype != torch.complex128):
            raise Exception("The data type of the matrix is not correct. Check the code again")
        
        self.alpha_bar_mat = final_mat

        # np.save("alpha_bar_mat.npy",self.alpha_bar_mat.numpy())
        return(self.alpha_bar_mat)
    
    def J_i_j(self,input_variable:input_variables,delta_r:torch.Tensor,Gamma_b:torch.Tensor)->torch.Tensor:
        # Check if this gives the correct ouput
        ...
        matrix_1 = torch.einsum('k,ijk->ij',delta_r,alpha_i_j_mat)
        matrix_2 = torch.einsum('ijk,kl,ijl->ij',alpha_i_j_mat,Gamma_b,alpha_i_j_mat)

        try:
            self.J_i_j_mat = J_0*torch.exp(-1j*matrix_1)*torch.exp(-0.5*matrix_2)        
            if(torch.any(torch.isnan(self.J_i_j_mat))):
                self.J_i_j_nan_inf_flag = True
                print(" Exception: There is a NaN in the computation of the J_i_j matrix. Figure out why this is happening.")
    ...
    def chemical_potential(self,input_variable:input_variables)->torch.Tensor:
        ...
        final_matrix = torch.diag(input_variable.chemical_potential_val*torch.ones((N_f,)) - 0.5*torch.diag(self.Ve_i_j_mat) ) 
        return final_matrix
    
    ##############################################################################
    ##############################################################################
    # Functions needed to describe the electron-phonon coupling part of the Hamiltonian
    def delta_gamma(self,input_variables:input_variables)->torch.Tensor:
        ...
        self.delta_gamma_mat = gamma - torch.einsum('kl,lj->kj',omega,lmbda) 
    ...
    def delta_gamma_tilda(self, input_variables:input_variables)->torch.Tensor:
        ...
        self.delta_gamma_tilde_mat[0:N_b,:] = self.delta_gamma_mat
    ...
    def initialize_all_variables(self,input_variables:input_variables,delta_r:torch.Tensor,Gamma_b:torch.Tensor):
